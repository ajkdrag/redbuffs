---
{"publish":true,"tags":["type/zettel","status/done"],"PassFrontmatter":true,"created":"2024-11-14T14:33:52.040+05:30"}
---
...
In 1-bit LLMs, we deal with binarized model weights ⎯ in [[BitNet\|BitNet]], it's binary `[-1, 1]`, while in [[2 Zettels/BitNet b1.58\|BitNet b1.58]], it's ternary `[-1, 0, 1]`. To perform this binarization, few non-differentiable operations such `round()` and `clip()` are employed in the layer. Since these are non-differentiable, we use tricks such as [[2 Zettels/straight through estimators\|straight through estimators]] to approximate the gradient during backpropagation.
