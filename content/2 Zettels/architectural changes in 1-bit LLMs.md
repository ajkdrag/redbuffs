...
In a [[transformer\|transformer]] block, the feed-forward layer is the most computationally heavy. In 1-bit LLMs, the `nn.Linear` is replaced with a `BitLinear` layer. This layer essentially performs a `RoundClip` operation converting regular float values to -1, 0 or 1.  We also [[2 Zettels/quantize activation functions\|quantize activation functions]] to 8-bit precision in both [[BitNet\|BitNet]] and [[2 Zettels/BitNet b1.58\|BitNet b1.58]].
