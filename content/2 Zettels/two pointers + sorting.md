...
**Problem**: (2Sum) Find two indices in a sorted array where elements sum to target.
**Approach**:
...
**Problem**: Return the deduplicated array.
**Approach**:
...
**Problem**: (3Sum) Find all triplets `[a, b, c]` such that `a + b + c = 0`.
**Approach**:
- Sort the array $O(n \log n)$.
